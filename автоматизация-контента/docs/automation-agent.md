...
`schedule run` без `--date` берёт **сегодня + все просроченные** `scheduled`/`pending` (catch-up). Dry-run без `AUTO_PUBLISH` слоты в `done` не переводит. Если статус кампании не `approved` — слот остаётся в очереди, не уходит в `failed`.

## Таймер

Cron UTC `0 7,8,9,10,11 * * *` → 10:00–14:00 МСК: обложки, sync-urls, слоты, повтор pending + catch-up.
