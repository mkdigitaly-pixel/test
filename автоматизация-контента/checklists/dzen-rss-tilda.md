...
### Если Студия пишет «не соответствует требованиям к контенту»

Дзен считает неоригинальным:

- копии публикаций, которые **уже есть в канале**;
- тексты, почти полностью совпадающие с другими сайтами;
- страницы «только для RSS», закрытые от посетителей.

В ленте должны быть **свои статьи блога**, которых ещё нет в канале. Минимум **10 материалов** при первой заявке. Проверок RSS — не больше **трёх в год**; заимствованный контент проверяют один раз.

После правки ленты в Студии снова нажмите отправку RSS на проверку (агент это нажать не может).

**Зачем:** иначе заявка отклоняется, даже если XML валидный.
