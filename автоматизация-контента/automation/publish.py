...
def publish_standalone_vk(post_id: str, *, dry_run: bool, force: bool) -> int:
    ...
    if not dry_run and (not vk_token or not vk_group):
        raise SystemExit("Заполните VK_ACCESS_TOKEN и VK_GROUP_ID")
    r = publish_vk(
        text,
        vk_token or "dry",
        vk_group or "0",
...
def cmd_publish(target: str, item_id: str, *, dry_run: bool, force: bool) -> int:
    ...
    if target in ("teasers", "teasers-vk", "all"):
        ...
        if not vk_only and not dry_run:
            if not token or not main_ch:
                raise SystemExit("Заполните TELEGRAM_BOT_TOKEN и TELEGRAM_MAIN_CHANNEL_ID")
        if vk_only or dry_run or (token and main_ch):
...
def cmd_schedule_run(args: argparse.Namespace) -> int:
    ...
    for slot in slots:
        ...
        if when.date() > day:
            continue
        if when.date() == day and not args.force and when > now:
            continue
        ...
        if ok:
            if dry:
                print("  ℹ dry-run — статус слота не меняем")
            else:
                slot["status"] = "done"
                slot["completed_at"] = now.isoformat()
                ran += 1
        elif dry:
            # Не портим очередь отладочным прогоном без токенов
            print("  ℹ dry-run — ошибка не пишет failed")
        elif "нужен approved" in msg:
...
def cmd_schedule_prepare_covers(args: argparse.Namespace) -> int:
    ...
    for slot in data.get("slots", []):
        if slot.get("status") not in ("scheduled", "pending"):
            continue
        when = slot_when(slot)
        # Как schedule run: сегодня + просроченные
        if when.date() > day:
            continue
        action = slot.get("action", "")
        cid = slot.get("campaign_id") or slot.get("post_id") or ""
